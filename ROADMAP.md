...
**Current state (2026-05-01):** PT-V3 (31.8M params) trained 25 epochs; val seg_acc 49.5%, axis_deg 38.3°, valid_acc 96.9%. Phase D pipeline integration complete: end-to-end inference, semi-automatic user annotation GUI (rect + lasso + camera Tab cycle, strict-mode merge, persisted annotations), motion-image refinement (wired but bottlenecked on calibration), **D.4 geometric joint extraction landed** (5/6 joints recovered correctly on test_2 vs ML's ~0/6, bypasses the model's 38° axis error entirely), and the interactive script auto-emits a `user_annotation.glb` + `refined_assembled.glb` pair for one-click rough-vs-refined comparison. **D.2 retrieval was attempted and abandoned** (PT-V3 global features are pose-sensitive and biased — cosine sim ranked KUKA IIWA above the actual xArm6 input on test_2). **D.6 joint limits as a model output is in progress**: 13-row `urdf_db.json` template lookup deleted; LimitsHead added; v3 training shards embedding per-joint `(lower, upper)` are regenerating (MJCF done at 343 shards / 10,950 examples; URDF in progress); PyBullet self-collision sweep at inference refines model-predicted limits with adjacent-pair filtering. **Next: complete v3 regen, retrain PT-V3 on v3 (H200), end-to-end test of model-limits → sweep → URDF on test_2.**
